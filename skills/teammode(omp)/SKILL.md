...
4. Dispatch all members in one `task` call using the host batch form: a `tasks[]` array (one `TaskItem` per member with `name`, `agent`, `task`, and `isolated: true` for worktree isolation) plus a shared `context` string carrying the team brief. Set `blocking: false`. Each worker receives a compact task-packet with a tier-appropriate budget. Do not replace a missing asynchronous path with synchronous execution and do not claim that inline results form a team.
