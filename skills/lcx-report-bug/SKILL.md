...
2. Route to `omp-lazy` when the failure depends on this package. Route to `omp` only when it reproduces in clean OMP 16.4.8. Stop on ambiguous ownership or an explicit target mismatch.
