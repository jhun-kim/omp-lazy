...
1. Confirm that the active OMP provider exposes `task`, `job`, and `irc`. Stop with `async_team_surfaces_unavailable` if any surface is absent.
...
6. Coordinate through `irc` using bound actual IDs. Poll or cancel only bound job IDs. Never interrupt an arbitrary agent.
